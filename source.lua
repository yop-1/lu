...
local function BlurModule(Frame)
    ...
    local MTREL = "Glass"
    local binds = {}
    local root = camera:FindFirstChild('LunaBlur')
    if root then
        -- If already exists, return early to avoid duplicates
        return
    end
    root = Instance.new('Folder', camera)
    root.Name = 'LunaBlur'

    local gTokenMH = 99999999
    local gToken = math.random(1, gTokenMH)

    -- Reduce DepthOfField intensity drastically to avoid blurry arms
    local DepthOfField = Instance.new('DepthOfFieldEffect', game:GetService('Lighting'))
    DepthOfField.FarIntensity = 0
    DepthOfField.FocusDistance = 70 -- push focus farther out
    DepthOfField.InFocusRadius = 100
    DepthOfField.NearIntensity = 1 -- much lower than original 6
    DepthOfField.Name = "DPT_"..gToken
    DepthOfField.Enabled = false  -- start disabled, enable only when needed
